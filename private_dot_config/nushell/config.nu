...
# Keep mise from snapshotting project-local PATH entries managed by direnv.
# Shims resolve the configured tool version without rewriting PATH on each
# prompt or directory change.
...
const my_cubrid_aliases = if ('~/my-cubrid/aliases.nu' | path expand | path exists) {
  '~/my-cubrid/aliases.nu'
} else {
  null
}
source $my_cubrid_aliases
...
let v = ($env.HOME | path join "vcpkg")
if ($v | path exists) {
  $env.VCPKG_ROOT = $v
  path add $v
}
